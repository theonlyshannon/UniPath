            ...
            <div class="row gutter-y-50 align-items-center">
                <div class="col-lg-6 wow fadeInUp" data-wow-duration="1500ms" data-wow-delay="00ms">
                    <div class="select-interests-page__image">
                        <img src="{{ asset('app/images/student-2.jpg') }}" alt="select interests"
                            class="select-interests-page__image__inner">
                    </div>
                </div>
                ...
                <div class="col-lg-6 wow fadeInUp" data-wow-duration="1500ms" data-wow-delay="100ms">
                    <form action="{{ route('student.select-interests.store') }}" method="POST">
                        ...
                        <!-- Universitas Pilihan -->
                        <div class="mb-3">
                            <x-forms.select label="Universitas Pilihan" name="university_main_id"
                                id="university_main_id" :options="$universities" key="id" value="name"
                                :selected="old('university_main_id', auth()->user()->student->university_main_id ?? '')" required />
                        </div>

                        <!-- Fakultas Pilihan -->
                        <div class="mb-3">
                            <x-forms.select label="Fakultas Pilihan" name="faculty_main_id" id="faculty_main_id"
                                :options="$faculties" key="id" value="name" :selected="old('faculty_main_id', auth()->user()->student->faculty_main_id ?? '')" required />
                        </div>

                        <!-- Universitas Cadangan (Opsional) -->
                        <div class="mb-3">
                            <x-forms.select label="Universitas Cadangan (Opsional)" name="university_second_id"
                                id="university_second_id" :options="$universities" key="id" value="name"
                                :selected="old(
                                    'university_second_id',
                                    auth()->user()->student->university_second_id ?? '',
                                )" />
                        </div>

                        <!-- Fakultas Cadangan (Opsional) -->
                        <div class="mb-3">
                            <x-forms.select label="Fakultas Cadangan (Opsional)" name="faculty_second_id"
                                id="faculty_second_id" :options="$faculties" key="id" value="name"
                        ...
                        <div class="mb-3">
                            ...
                            <x-forms.select label="Jenis Kelamin" name="gender" id="gender" :options="$genders"
                                key="value" value="label" required />
                        </div>
